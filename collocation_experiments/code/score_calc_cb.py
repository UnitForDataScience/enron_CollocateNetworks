...
collocation_file = ""
ind_frequency_file = ""
words_to_include_file = "wordstoinclude.txt"
t_score_output_file = ''
...
# decides if either both words (1) or just a single word (0) should be present in the code book
wordlistind = 1
...
def load_values():
	...
	print 'loading frequencies and word list'
	print 'reading file '+collocation_file
	...
	print 'reading file '+ind_frequency_file
	...
	with open(words_to_include_file,'r') as f:
		for line in f.readlines():
			sw_list.append(line.strip().lower())
	print 'reading wordstoinclude list complete. Total count: '+str(len(sw_list))
...
def scoreCal():
	...
	for key, value in cnt_items:
		...
		sys.stdout.flush()
		words = key.split('-')
		if wordlistind == 1 and (words[0].strip().lower() not in sw_list or words[1].strip().lower() not in sw_list):
			continue
		elif wordlistind == 0 and words[0].strip().lower() not in sw_list and words[1].strip().lower() not in sw_list:
			continue
		fl = ind_list[words[0]]
		fr = ind_list[words[1]]
		den = sqrt(value)
	...
	print ''
	count =0
	sys.stdout.write("Writing to %s: %.2f%% complete\r"%(t_score_output_file,float(count*100.0/new_count)))
	sys.stdout.flush()
	...
	print ''
	count =0
	sys.stdout.write("Writing to %s: %.2f%% complete\r"%(mi_score_output_file,float(count*100.0/new_count)))
	sys.stdout.flush()
...
def init():
	global collocation_file, ind_frequency_file, ts_cutoff_indicator, mi_cutoff_indicator, ts_cutoff, mi_cutoff, t_score_output_file, mi_score_output_file, words_to_include_file, wordlistind
	...
	words_to_include_file = raw_input('Enter codebook list file path:\n')
	wordlistind = int(raw_input('Do you need both words of the pairing to be present in codebook (1/0)\n'))
